...
4. **Populate the dashboard now - you run the first sweep yourself; do not wait on the worker.** A brand-new install has an empty board, and this is the step that fills it. Understand why you must do it yourself: the every-minute Attention Major worker CANNOT run while you (this setup session) are the active agent, because Scout runs one agent session at a time. So queuing a sweep and waiting for the worker just stalls on a blank board - the very bug this step exists to prevent. Check `m_m365_status`:
