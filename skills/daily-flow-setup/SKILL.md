...
4. **Seed the dashboard now (first run).** A brand-new install has an empty board, so populate it immediately. Check `m_m365_status`: if the user is signed in to Microsoft 365, POST `http://127.0.0.1:<port>/api/attention-major` with body `{"source":"setup-wizard","force":true}` to queue one broad sweep. The every-minute Attention Major worker picks it up within about a minute and fills the board with their real email, Teams, invites, and meeting prep; tell the user it will fill within a minute. If they are not signed in, skip the sweep and tell them the board fills once they sign in to Microsoft 365 and the next pulse runs.

## Step 8 - Verify and hand off
- GET `/api/state` and confirm healthy. Confirm via `m_list_automations` that all four automations exist and are enabled, and that each stored prompt matches the file (the Step 6 verify). Confirm the first-run sweep was queued, or was correctly skipped because the user is not yet signed in to Microsoft 365.
